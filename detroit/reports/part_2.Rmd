...
joined = 
  dplyr::tbl(con, 'assessments') %>%
  inner_join(dplyr::tbl(con, 'sales'), by = c('PARCELNO'='parcel_num'), suffix = c("_assessed","")) %>%
  # left_join(dplyr::tbl(con, 'parcels'), by = c('parcel_num'='parcel_number'), suffix = c("", "_parcel")) %>%
  dplyr::collect()
...
head(assessments_2016)
```

```{r}
assessments_B =
  assessments_2016 %>%
  left_join(parcels, by = c('PARCELNO'='parcel_number'))
